...
public class ServerRepositoryImp implements ServerRepository {
    ...
    @Override
    public SystemModule getSystem(int id) {
    ...
    @Override
    @ReturnInsert
    public GetRequest createGetRequest(GetRequest gerRequest) {
    ...
    public GetRequest updateGetRequest(GetRequest getRequest) {
        GetRequest getRequestExists = em.find(GetRequest.class, getRequest.getId());
        if (getRequest == null) {
            return null;
        }
        em.merge(getRequest);
        em.flush();
        em.refresh(getRequestExists);
